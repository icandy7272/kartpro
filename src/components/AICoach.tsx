...
function generateClipboardReport(analyses: LapAnalysis[]): string {
  ...
  let text = `=== KartPro 训练数据 ===\n`
  text += `总圈数: ${laps.length} | 最快圈: 第${fastest.id}圈 ${formatDuration(fastest.duration)} | 最慢圈: 第${slowest.id}圈 ${formatDuration(slowest.duration)}\n`
  text += `圈速差: ${(slowest.duration - fastest.duration).toFixed(3)}s | 平均圈速: ${formatDuration(mean)} | 标准差: ${stdDev.toFixed(3)}s | 最高速度: ${topSpeed.toFixed(1)} km/h\n\n`

  // Lap times list
  text += `--- 各圈圈速 ---\n`
  for (const a of analyses) {
    const delta = a.lap.duration - fastest.duration
    const mark = a.lap.id === fastest.id ? ' ★最快' : ` +${delta.toFixed(3)}s`
    text += `第${a.lap.id}圈: ${formatDuration(a.lap.duration)}${mark}\n`
  }

  // Corner info from fastest lap
  if (fastestAnalysis.corners.length > 0) {
    text += `\n--- 弯道分析（最快圈）---\n`
    text += `弯道 | 方向 | 类型 | 角度 | 入弯 | 最低 | 出弯 (km/h)\n`
    for (const corner of fastestAnalysis.corners) {
      ...
      const dir = totalChange < 0 ? '左' : '右'
      const angle = Math.abs(totalChange)
      const type = angle > 160 ? '掉头弯' : angle > 120 ? '发卡弯' : angle > 60 ? '中速弯' : angle > 20 ? '高速弯' : '快速变向'
      text += `${corner.name} | ${dir} | ${type} | ${Math.round(angle)}° | ${Math.round(corner.entrySpeed)} | ${Math.round(corner.minSpeed)} | ${Math.round(corner.exitSpeed)}\n`
    }
    ...
    for (const a of analyses) {
      const delta = a.lap.duration - fastest.duration
      const mark = a.lap.id === fastest.id ? ' ★' : ''
      text += `\n第${a.lap.id}圈 (${formatDuration(a.lap.duration)}, ${delta >= 0 ? '+' : ''}${delta.toFixed(3)}s)${mark}\n`
      for (const c of a.corners) {
        const fastCorner = fastestAnalysis.corners.find((fc) => fc.name === c.name)
        const timeDelta = fastCorner && c.duration && fastCorner.duration ? (c.duration - fastCorner.duration).toFixed(3) : '-'
        text += `  ${c.name}: 入弯 ${Math.round(c.entrySpeed)} km/h, 最低 ${Math.round(c.minSpeed)} km/h, 出弯 ${Math.round(c.exitSpeed)} km/h, 耗时 ${c.duration?.toFixed(3) || '-'}s [${timeDelta}s]\n`
      }
    }
  }
